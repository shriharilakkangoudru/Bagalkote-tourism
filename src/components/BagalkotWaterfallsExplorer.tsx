...
export const BagalkotWaterfallsExplorer: React.FC<BagalkotWaterfallsExplorerProps> = ({
  ...
  const waterfallPhotos = activeWaterfall.galleryImages && activeWaterfall.galleryImages.length > 0
    ? activeWaterfall.galleryImages
    : [activeWaterfall.image];

  // Stop audio on change or unmount and reset photo index
  useEffect(() => {
    stopAllSpeech();
  ...
  useEffect(() => {
    if (!isAutoPlaySlides || waterfallPhotos.length <= 1) {
      setSlideProgress(0);
      return;
    }
    ...
    const timer = setInterval(() => {
      setSlideProgress((prev) => {
        if (prev >= 100) {
          setActiveImageIndex((curr) => (curr + 1) % waterfallPhotos.length);
          return 0;
        }
  ...
  }, [isAutoPlaySlides, slideDuration, waterfallPhotos.length, activeWaterfallId]);

  const handleNextPhoto = () => {
    setActiveImageIndex((prev) => (prev + 1) % waterfallPhotos.length);
    setSlideProgress(0);
  };

  const handlePrevPhoto = () => {
    setActiveImageIndex((prev) => (prev - 1 + waterfallPhotos.length) % waterfallPhotos.length);
    setSlideProgress(0);
  };
      ...
      {viewMode === 'detail' ? (
        <div className="p-4 sm:p-6 lg:p-8 space-y-8 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            {/* Left Column: Photo Showcase & Gallery with 5-6s Slideshow */}
            <div className="lg:col-span-7 space-y-4">
              <div className="relative rounded-2xl overflow-hidden aspect-[16/10] bg-[#0c1320] border border-cyan-500/30 shadow-xl group select-none">
                <img
                  src={waterfallPhotos[activeImageIndex] || activeWaterfall.image}
                  alt={`${activeWaterfall.name} photo ${activeImageIndex + 1}`}
                ...
                <div className="absolute top-3 inset-x-3 flex items-center justify-between z-10 gap-2">
                  <div className="flex flex-wrap gap-2">
                    <span className="px-3 py-1 rounded-full bg-black/70 backdrop-blur-md text-xs font-bold text-cyan-300 border border-cyan-400/30">
                      {activeWaterfall.category}
                    </span>
                    <span className="px-3 py-1 rounded-full bg-black/70 backdrop-blur-md text-xs text-gray-200 border border-white/20 flex items-center gap-1">
                  ...
                  {waterfallPhotos.length > 1 && (
                    <div className="flex items-center gap-1.5 bg-black/80 backdrop-blur-md px-2 py-1 rounded-xl border border-cyan-400/30">
                      <button
                        onClick={() => setSlideDuration((d) => (d === 5000 ? 6000 : 5000))}
                        className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-cyan-500/20 text-cyan-300 hover:bg-cyan-500/30 transition-colors border border-cyan-500/40"
                      ...
                      >
                        {isAutoPlaySlides ? <Pause className="w-3.5 h-3.5 text-cyan-400" /> : <Play className="w-3.5 h-3.5 text-emerald-400" />}
                      </button>
                    </div>
                  )}
                </div>
                ...
                {waterfallPhotos.length > 1 && (
                  <>
                    <button
                      ...
                      className="absolute left-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-black/60 hover:bg-cyan-500/90 text-white hover:text-gray-950 backdrop-blur-md border border-white/20 flex items-center justify-center transition-all duration-200 shadow-lg active:scale-95"
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 z-20 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-black/60 hover:bg-cyan-500/90 text-white hover:text-gray-950 backdrop-blur-md border border-white/20 flex items-center justify-center transition-all duration-200 shadow-lg active:scale-95"
                ...
                {/* 5-6s Progress Bar */}
                {waterfallPhotos.length > 1 && isAutoPlaySlides && (
                  <div className="absolute bottom-0 inset-x-0 h-1 bg-black/60 z-20">
                    <div
                      className="h-full bg-gradient-to-r from-cyan-500 to-emerald-400 transition-all duration-100 ease-linear shadow-[0_0_8px_rgba(6,182,212,0.8)]"
                ...
                <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-xs text-gray-200 bg-black/60 backdrop-blur-md p-2.5 rounded-xl border border-white/10 z-10">
                  <span className="font-semibold truncate">
                    Photo {activeImageIndex + 1} of {waterfallPhotos.length}: {activeWaterfall.name}
                  </span>
                    ...
                    <button
                      ...
                      className={`relative w-20 h-14 rounded-xl overflow-hidden border-2 transition-all cursor-pointer shrink-0 ${
                        activeImageIndex === idx
                          ? 'border-cyan-400 scale-105 shadow-md shadow-cyan-500/30 ring-2 ring-cyan-400/40'
