...
import { MapPin, Clock, Camera, Users, Accessibility, Ticket, ChevronLeft, Navigation, MessageSquareText, Calendar, Sparkles, Check, Share2, Info, Globe, ExternalLink, Search, Image as ImageIcon } from 'lucide-react';
...
export const MonumentDetailsView: React.FC<MonumentDetailsViewProps> = ({
  ...
  const [selectedGalleryImage, setSelectedGalleryImage] = useState<string>(monument.image);

  // Immediately synchronize internal state whenever monument prop changes
  useEffect(() => {
    setSelectedGalleryImage(monument.image);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 text-white">
      ...
      <div className="relative rounded-3xl overflow-hidden border-2 border-amber-500/40 shadow-2xl bg-black">
        <img
          src={selectedGalleryImage || monument.image}
          alt={monument.name}
          className="w-full h-80 sm:h-[450px] object-cover transition-all duration-500"
        ...
        {/* Thumbnail selector */}
        {monument.galleryImages && monument.galleryImages.length > 1 && (
          <div className="absolute bottom-4 right-4 flex space-x-2 z-20">
            {monument.galleryImages.map((img, idx) => (
              <button
                key={idx}
                onClick={() => setSelectedGalleryImage(img)}
                className={`w-14 h-10 rounded-lg overflow-hidden border-2 transition-all ${
                  selectedGalleryImage === img ? 'border-amber-400 scale-105 shadow-md' : 'border-white/40 opacity-70'
                }`}
              >
                <img
                  src={img}
